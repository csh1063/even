...
public actor FaceEmbeddingService {
    ...
    public func extractEmbeddings(from image: CGImage, hasGlass: Bool = false) async -> [FaceEmbedding] {
        ...
        for observation in observations {
            // 내부에서 자동으로 안경을 체크하거나 외부 옵션 파라미터를 결합하여 유연하게 대처
            if let embedding = extractEmbedding(from: image, observation: observation) {
                // 외부에서 안경 체크를 강제했거나 내부 분류기가 안경을 찾아냈을 때의 필터링 제어
                if hasGlass && !embedding.hasGlasses {
                    continue // 안경 쓴 사람만 뽑고 싶을 때 예외처리 규칙
                }
    ...
    private func extractEmbedding(from image: CGImage, observation: VNFaceObservation) -> FaceEmbedding? {
        // 💡 해결책 1: 원본 이미지 상태에서 눈 위치를 기준으로 먼저 수평 정렬(Alignment)을 수행합니다.
        ...
        // 💡 해결책 2: 완벽하게 정렬된 'alignedImage'에서 최종 얼굴 구역을 크롭합니다.
        guard let cropped = cropFace(from: alignedImage, boundingBox: observation.boundingBox) else {
            return nil
        }
        ...
        // 크기 조정 및 임베딩 모델 실행
        guard let resized = resize(image: cropped, to: inputSize),
    ...
    private func alignFace(image: CGImage, landmarks: VNFaceLandmarks2D, boundingBox: CGRect) -> CGImage? {
        ...
        // 💡 정정: 원본 이미지 전체를 기준으로 픽셀 좌표를 산출하도록 변환식을 바로잡습니다.
        func toOriginalPixel(_ point: CGPoint) -> CGPoint {
            let actualX = (boundingBox.minX + (point.x * boundingBox.width)) * imageWidth
            let actualY = (1.0 - (boundingBox.minY + (point.y * boundingBox.height))) * imageHeight
        ...
        let faceCenterY = (1.0 - (boundingBox.minY + boundingBox.height / 2)) * imageHeight
